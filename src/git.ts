...
export function fetchOrigin(repoRoot: string, env: NodeJS.ProcessEnv): void {
  run("git", ["-C", repoRoot, "fetch", "origin", "--quiet"], { env });
}

export async function ensureWorktree(options: EnsureWorktreeOptions): Promise<WorktreeResult> {
  ...
  if (
    run("git", ["-C", options.repoRoot, "check-ref-format", "--branch", branch], {
      env: options.env,
    }).status !== 0
  ) {
  ...
  const commonDirRaw = run("git", ["-C", options.repoRoot, "rev-parse", "--git-common-dir"], {
    env: options.env,
  });
  const commonDir =
    commonDirRaw.status === 0 ? commonDirRaw.stdout.trim() : join(options.repoRoot, ".git");
  const absoluteCommonDir = isAbsolute(commonDir) ? commonDir : join(options.repoRoot, commonDir);
  ...
  try {
    ...
    run("git", ["-C", options.repoRoot, "worktree", "prune"], { env: options.env });
    addWorktree(options.repoRoot, worktreePath, branch, options.env);
  } finally {
    await releaseLock(lockDir);
  }

  const toplevel = runRequired("git", ["-C", worktreePath, "rev-parse", "--show-toplevel"], {
    env: options.env,
  });
...
function reuseExistingWorktree(
  worktreePath: string,
  branch: string,
  env: NodeJS.ProcessEnv,
  ...
  const toplevel = run("git", ["-C", worktreePath, "rev-parse", "--show-toplevel"], { env });
  if (toplevel.status !== 0 || toplevel.stdout.trim() !== worktreePath) {
    throw new CliError(`${worktreePath} already exists and is not a git worktree`);
  }
  ...
  return { branch: headBranch.status === 0 ? headBranch.stdout.trim() : branch, worktreePath };
}
...
function addWorktree(
  ...
  if (
    run("git", ["-C", repoRoot, "show-ref", "--verify", "--quiet", `refs/heads/${branch}`], { env })
      .status === 0
  ) {
    runRequired("git", ["-C", repoRoot, "worktree", "add", worktreePath, branch], { env });
    return;
  }
  ...
  const defaultBranch =
    defaultBranchOutput.status === 0
      ? defaultBranchOutput.stdout.trim().replace(/^origin\//, "")
      : "";
  if (!defaultBranch) {
    throw new CliError(
  ...
  if (
    run("git", ["-C", repoRoot, "rev-parse", "--verify", `${baseRef}^{commit}`], { env }).status !==
    0
  ) {
    throw new CliError(`missing ${baseRef}`);
  }

  runRequired("git", ["-C", repoRoot, "worktree", "add", "-b", branch, worktreePath, baseRef], {
    env,
  });
}
